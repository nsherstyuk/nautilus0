...
<#
.SYNOPSIS
    Phase 6 Configuration Update Script for Windows
...
.EXAMPLE
    .\optimization\scripts\run_phase6_config_update.ps1 -DryRun -Verbose

.EXAMPLE
    .\optimization\scripts\run_phase6_config_update.ps1 -SensitivityJson "custom_sensitivity.json" -Phase6Yaml "custom_config.yaml"
...
function Validate-UpdatedConfig {
    if ($DryRun) {
        Write-ColorOutput "DRY RUN: Would validate updated configuration" $ColorYellow
        return
    }
    # Lightweight validation: ensure file exists
    $targetFile = if ($OutputYaml) { $OutputYaml } else { $Phase6Yaml }
    if (-not (Test-Path $targetFile)) {
        Write-ColorOutput "ERROR: Updated configuration file not found: $targetFile" $ColorRed
        exit 4
    }
    Write-ColorOutput "✓ Updated configuration file present: $targetFile" $ColorGreen
}

function Show-UpdateSummary {
    Write-ColorOutput "`nPhase 6 Configuration Update Summary" $ColorGreen
    Write-ColorOutput "====================================" $ColorGreen
    Write-ColorOutput "Configuration file has been updated. Review the YAML for details." $ColorWhite
}
...
function Show-NextSteps {
    ...
    Write-ColorOutput "   Get-Content optimization/configs/phase6_refinement.yaml" $ColorYellow
