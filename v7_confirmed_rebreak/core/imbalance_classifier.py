...
class ImbalanceClassifier:
    ...
    Internal complexity hidden:
        - Volume accumulation buffer
        - Zero-volume edge cases
        - Direction-aware divergence/matching logic
    """

    def __init__(self, max_lookback: int = 10, min_bar_ticks: int = 0):
        self._buy_vols = deque(maxlen=max_lookback)
        self._sell_vols = deque(maxlen=max_lookback)
        self._tick_counts = deque(maxlen=max_lookback)
