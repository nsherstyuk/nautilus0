"""
V7 Confirmed Rebreak -- Live Trading Script

Connects to IBKR via ib_insync, streams 1-min bars, feeds them into
the LiveEngine which detects confirmed rebreak patterns and manages trades.

Bar sourcing:
  1. Seed rolling buffer with recent historical 1-min bars from IBKR
  2. Stream real-time updates via reqMktData (since XAUUSD CMDTY does not
     support reqTickByTickData)
  3. Aggregate price updates into 1-min bars with buy/sell classification
     (uptick = buy, downtick = sell)

Usage:
  python -m v7_confirmed_rebreak.live.run_live --dry-run
  python -m v7_confirmed_rebreak.live.run_live --port 4002
"""
...
from v7_confirmed_rebreak.live.live_config import LiveConfig
from v7_confirmed_rebreak.live.live_engine import LiveEngine, Bar
...
def setup_logging(log_dir: Path) -> logging.Logger:
    ...
    fh = logging.FileHandler(str(log_dir / f"v7_live_{ts}.log"))
    fh.setFormatter(fmt)
...
class IBKRConnection:
    """IBKR connection manager for V7 live trading."""
    ...
    def get_mid_price(self) -> Optional[float]:
        """Get current mid price from streaming data."""
    ...
    def fetch_historical_bars(self, duration: str = "1 D",
        ...
        """Fetch historical 1-min bars to seed the rolling buffer."""
...
class BarAggregator:
    ...
    Not perfect, but sufficient for the divergence/matching imbalance filter.
    """
    ...
    def on_price(self, price: float, now: datetime) -> Optional[Bar]:
        ...
        if self.current_bar_start is not None and bar_start > self.current_bar_start:
            # Minute boundary crossed -- close current bar
        ...
        # Classify as buy/sell based on price direction
...
class V7LiveTrader:
    ...
    def __init__(self, live_cfg: LiveConfig, log: logging.Logger):
        ...
        self.strat_cfg = StrategyConfig(
            instrument=live_cfg.symbol,
            pivot_window=live_cfg.pivot_window,
            confirm_bars=live_cfg.confirm_bars,
            sl_atr_multiple=live_cfg.sl_atr_multiple,
            tp_atr_multiple=99.0,
            max_hold_bars=live_cfg.max_hold_bars,
            min_bar_ticks=live_cfg.min_bar_ticks,
            spread_cost=live_cfg.spread_cost,
        )

        self.conn = IBKRConnection(live_cfg, log)
        self.engine = LiveEngine(live_cfg, self.strat_cfg, log)
        self.aggregator = BarAggregator(log)
        self.sl_order = None
        self._shutdown = False

        # State persistence
        self.state_dir = ROOT / "v7_confirmed_rebreak" / "live" / "state"
        self.state_dir.mkdir(parents=True, exist_ok=True)
        self.log_dir = ROOT / "v7_confirmed_rebreak" / "live" / "logs"
        self.log_dir.mkdir(parents=True, exist_ok=True)
        self.trade_log_path = self.log_dir / "trades.csv"

    def seed_buffer(self):
        """Load historical bars to seed the rolling buffer."""
        # Try to load enough bars for pivot computation
    ...
    def run(self):
        ...
        self.log.info(f"Engine ready. Buffer: {len(self.engine.buffer.bars)} bars. "
        ...
        while not self._shutdown:
            ...
            if completed_bar is not None:
                # New bar completed -- feed to engine
                result = self.engine.on_bar(completed_bar)

                # Log bar info periodically
                bars_in_buf = len(self.engine.buffer.bars)
                ...
                if result is not None:
                    self._handle_signal(result, completed_bar)
        ...
        self._cleanup()
        self.conn.disconnect()
        self.log.info("V7 live trader stopped.")

    def _handle_signal(self, result: dict, bar: Bar):
        """Handle engine signal (entry or exit)."""
        if 'action' in result and result['action'] == 'exit':
            self._handle_exit(result)
            return

        if 'direction' in result:
            self._handle_entry(result, bar)

    def _handle_entry(self, signal: dict, bar: Bar):
        ...
        if self.live_cfg.dry_run:
            ...
            trade = self.engine.enter_trade(signal, bar)
            return
        ...
        if trade:
            self.sl_order = self.conn.submit_stop_order(
                direction, self.live_cfg.quantity, trade.sl_price)

    def _handle_exit(self, exit_info: dict):
        """Execute exit."""
        reason = exit_info['reason']
        self.log.info(f"EXIT SIGNAL: reason={reason}")

        trade_result = self.engine.exit_trade(exit_info)
        if not trade_result:
            return
        ...
        log_row = {
            ...
            'sl_price': f"{trade_result['sl_price']:.2f}",
            'pivot_price': f"{trade_result['pivot_price']:.2f}",
            'quantity': trade_result['quantity'],
            'pnl': f"{trade_result['pnl']:.2f}",
            'exit_reason': trade_result['exit_reason'],
            'buy_ratio': f"{trade_result['buy_ratio']:.3f}",
            'gap': trade_result['gap'],
        }
        ...
        if reason != 'SL':
            self.conn.close_position(
                trade_result['direction'], self.live_cfg.quantity)

    def _cleanup(self):
        """Clean up on shutdown."""
        if self.engine.current_trade is not None:
            self.log.warning("Open trade at shutdown -- closing at market")
            if not self.live_cfg.dry_run:
                trade = self.engine.current_trade
                self.conn.close_position(trade.direction, trade.quantity)
                if self.sl_order:
                    try:
                        self.conn.ib.cancelOrder(self.sl_order.order)
                    except Exception:
...
def main():
    parser = argparse.ArgumentParser(description="V7 Confirmed Rebreak -- Live")
    ...
    log_dir = ROOT / "v7_confirmed_rebreak" / "live" / "logs"
    log = setup_logging(log_dir)

    log.info("="*60)
    log.info("V7 Confirmed Rebreak -- Live Trader")
    ...
    trader = V7LiveTrader(live_cfg, log)
    trader.run()
