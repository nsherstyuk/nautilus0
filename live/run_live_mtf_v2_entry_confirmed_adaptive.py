"""
Live MTF V2 Trading - Entry Confirmed ADAPTIVE version.

This live runner uses the ADAPTIVE entry confirmation strategy that matches
the backtest performance from run_backtest_mtf_v2_entry_confirmed_adaptive.py.

Key features:
- Uses MLSignalStrategyV2EntryConfirmedAdaptive (adaptive_full strategy)
- High confidence bypass threshold (reduces confirmation for high-confidence signals)
- Volatility-based adjustment (adapts confirmation to market conditions)
- Dynamic bars/threshold based on ATR and signal confidence
- Hour exclusions (weekday-specific)
- All settings match backtest configuration
...
logger = logging.getLogger("live_v2_entry_confirmed_adaptive")
...
class PortfolioFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if record.levelno == logging.DEBUG:
            if record.name.startswith("TRADER-V2-EC-ADAPTIVE.InteractiveBrokersClient-"):
                return False
            if record.name.startswith("TRADER-V2-EC-ADAPTIVE.ExecEngine"):
                return False
...
def setup_logging(log_dir: Path, start_time: str) -> logging.Logger:
    ...
    console_log_file = log_dir / f"console_adaptive_{start_time}.log"
    ...
    logging.getLogger("TRADER-V2-EC-ADAPTIVE.InteractiveBrokersClient-018").setLevel(logging.INFO)
    logging.getLogger("TRADER-V2-EC-ADAPTIVE.InteractiveBrokersClient-019").setLevel(logging.INFO)
    logging.getLogger("TRADER-V2-EC-ADAPTIVE.ExecEngine").setLevel(logging.INFO)

    log = logging.getLogger("live_v2_entry_confirmed_adaptive")
    log.info("V2 Entry Confirmed ADAPTIVE live logging configured. Logs directory: %s", log_dir)
    ...
    log_and_write_run_metadata(
        ...
        run_id=start_time,
        entrypoint=__file__,
        extra={"logger": "live_v2_entry_confirmed_adaptive", "log_dir": str(log_dir)},
    )
    return log
...
def main() -> int:
    """Entry point for V2 Entry Confirmed ADAPTIVE live trading."""
    ...
    logger.info("=" * 80)
    logger.info("MTF V2 LIVE TRADING - ADAPTIVE ENTRY CONFIRMED VERSION")
    logger.info("=" * 80)
    ...
    # Strategy config: USE ADAPTIVE STRATEGY (matches backtest)
    strategy_config = ImportableStrategyConfig(
        strategy_path="strategies.ml_strategy_mtf_v2_entry_confirmed_adaptive_full:MLSignalStrategyV2EntryConfirmedAdaptive",
        config_path="strategies.ml_strategy_mtf_v2_entry_confirmed_adaptive_full:MLSignalStrategyV2EntryConfirmedAdaptiveConfig",
        config={
    ...
    node_config = TradingNodeConfig(
        trader_id="TRADER-V2-EC-ADAPTIVE",
    ...
    logger.info("=" * 80)
    logger.info("V2 ADAPTIVE ENTRY CONFIRMED STRATEGY")
    logger.info("=" * 80)
    ...
    logger.info("Strategy: MLSignalStrategyV2EntryConfirmedAdaptive (ADAPTIVE)")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info("V2 ADAPTIVE Entry Confirmed live trading active. Press Ctrl+C to stop.")
        ...
        logger.exception("V2 ADAPTIVE Entry Confirmed live trading error: %s", exc)
        return_code = 1
    finally:
        logger.info("Shutting down V2 ADAPTIVE Entry Confirmed...")
        bar_streamer.disconnect()
        try:
            node.dispose()
        except Exception as e:
            logger.warning("Error disposing node: %s", e)
        logger.info("V2 ADAPTIVE Entry Confirmed live trading system stopped.")
