...
logger = logging.getLogger("live_v2_dashboard_supervisor_having_live_limit_entry")
...
def _load_config() -> SupervisorConfig:
    project_root = Path(__file__).resolve().parent.parent
    child_script = project_root / "live" / "run_live_mtf_v2_dashboard_having_live_limit_entry.py"
        ...
        while True:
            ...
            if proc is None or proc.poll() is not None:
                if proc is not None and proc.poll() is not None:
                    logger.warning(
                        "Child exited with code %s. Restarting after %ss...",
                        proc.returncode,
            ...
            if cfg.stale_enabled:
                ...
                if uptime >= float(cfg.min_uptime_before_restart_sec):
                    ...
                    if status_age is None or status_age > float(cfg.stale_max_status_age_sec):
                        ...
                        unhealthy = True
                        reason = f"bar_stale age_sec={bar_age}"
                    else:
                        reason = "OK"
                    ...
                    if stale_fail_count >= int(cfg.stale_consecutive_fail_checks):
                        logger.warning("Unhealthy child detected: %s -> restarting...", reason)
                        if proc is not None:
                            _graceful_stop_child(proc, grace_sec=30)
                            proc = None
                        time.sleep(max(1, cfg.restart_delay_sec))
            ...
            time.sleep(max(0.2, float(cfg.check_interval_sec)))

    except KeyboardInterrupt:
        logger.info("KeyboardInterrupt - stopping supervisor")
