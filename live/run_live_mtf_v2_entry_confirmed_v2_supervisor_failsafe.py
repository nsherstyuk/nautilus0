"""Supervisor for Live MTF V2 Entry Confirmed with FAIL-SAFE Trading

This supervisor script manages the fail-safe entry-confirmed live runner with:
...
def _setup_logging() -> None:
    ...
    try:
        ...
        log_and_write_run_metadata(
            ...
            run_id=run_id,
            entrypoint=__file__,
            extra={"logger": "live_v2_entry_confirmed_supervisor_failsafe"},
        )
    except Exception:
        pass
...
def load_config() -> SupervisorConfig:
    project_root = Path(__file__).resolve().parent.parent
    child_script = project_root / "live" / "run_live_mtf_v2_entry_confirmed_v2_failsafe.py"
