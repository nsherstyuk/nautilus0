"""Supervisor for the dashboard-enabled MTF V2 Entry Confirmed live runner.

This script starts `live/run_live_mtf_v2_entry_confirmed_v2_dashboard.py` as a child process and
...
logger = logging.getLogger("live_v2_entry_confirmed_dashboard_supervisor")
...
def _load_config() -> SupervisorConfig:
    project_root = Path(__file__).resolve().parent.parent
    child_script = project_root / "live" / "run_live_mtf_v2_entry_confirmed_v2_dashboard.py"
...
def _setup_logging() -> None:
    ...
    try:
        ...
        log_and_write_run_metadata(
            ...
            run_id=run_id,
            entrypoint=__file__,
            extra={"logger": "live_v2_entry_confirmed_dashboard_supervisor"},
        )
    except Exception:
        pass
...
def _get_git_info() -> Dict[str, str]:
    ...
    try:
        ...
        # Get file hashes
        strategy_file = Path(__file__).resolve().parent.parent / "strategies" / "ml_strategy_mtf_v2_entry_confirmed.py"
        supervisor_file = Path(__file__).resolve().parent.parent / "live" / "run_live_mtf_v2_entry_confirmed_v2_dashboard_supervisor.py"
