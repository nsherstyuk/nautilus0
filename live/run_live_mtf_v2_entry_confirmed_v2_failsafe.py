"""
Live MTF V2 Trading - Entry Confirmed with FAIL-SAFE Position Protection.

This version includes automatic position protection verification and fail-safe logic.

Key features:
- Uses MLSignalStrategyV2EntryConfirmedFailSafe with position protection.
- 30-second grace period after order submission.
- 3 consecutive failed protection checks required before emergency flatten.
- Periodic health monitoring every 15m bar.
- Emergency flatten if SL/TP orders missing after all checks.
...
logger = logging.getLogger("live_v2_entry_confirmed_failsafe")
...
class PortfolioFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if record.levelno == logging.DEBUG:
            ...
            if record.name.startswith("TRADER-V2-EC-FS-001.InteractiveBrokersClient-"):
                return False
            if record.name.startswith("TRADER-V2-EC-FS-001.ExecEngine"):
                return False
...
def setup_logging(log_dir: Path, start_time: str) -> logging.Logger:
    ...
    log_and_write_run_metadata(
        ...
        run_id=start_time,
        entrypoint=__file__,
        extra={"logger": "live_v2_entry_confirmed_failsafe", "log_dir": str(log_dir)},
    )
    return log
...
def main() -> int:
    ...
    # Strategy config: fail-safe version with position protection
    strategy_config = ImportableStrategyConfig(
        strategy_path="strategies.ml_strategy_mtf_v2_entry_confirmed_failsafe:MLSignalStrategyV2EntryConfirmedFailSafe",
        config_path="strategies.ml_strategy_mtf_v2_entry_confirmed_failsafe:MLSignalStrategyV2EntryConfirmedConfig",
        config={
    ...
    ok_15m = bar_streamer.subscribe_bars_sync(
        ...
        use_rth=False,
        duration="5 D",  # 5 days to ensure 50+ bars for warmup (12.5 hours + weekend buffer)
    ...
    ok_1m = bar_streamer.subscribe_bars_sync(
        ...
        callback=strategy_instance.on_bar,
        use_rth=False,
        duration="5 D",  # 5 days of 1m bars for entry confirmation
