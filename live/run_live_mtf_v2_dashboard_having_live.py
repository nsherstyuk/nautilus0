...
logger = logging.getLogger("live_v2_dashboard_having_live")
...
def setup_logging(log_dir: Path, start_time: str) -> None:
    ...
    log = logging.getLogger("live_v2_dashboard_having_live")
    ...
    log_and_write_run_metadata(
        ...
        run_id=start_time,
        entrypoint=__file__,
        extra={"logger": "live_v2_dashboard_having_live", "log_dir": str(log_dir)},
    )
...
def main() -> int:
    ...
    logger.info("=" * 80)
    logger.info("MTF V2 LIVE TRADING (DASHBOARD ENABLED, HAVING_LIVE)")
    logger.info("=" * 80)
    ...
    strategy_config = ImportableStrategyConfig(
        strategy_path="strategies.ml_strategy_mtf_v2_having_live:MLSignalStrategyV2HavingLive",
        config_path="strategies.ml_strategy_mtf_v2_having_live:MLSignalStrategyV2HavingLiveConfig",
        config={
            "order_id_tag": "V2",
    ...
    state: Dict[str, Any] = {
        ...
        "last_threshold": None,
        "guard": {"enabled": True, "grace_sec": float(protection_grace_sec), "require_open_orders": bool(protection_require_open_orders)},
