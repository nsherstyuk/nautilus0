"""
Optimize Positive Stall Detection Parameters - Using Working Replay Backtest

This script runs the optimization by directly using the working run_backtest_mtf_v2_replay.py
implementation. It modifies stall parameters programmatically without touching .env.mtf_v2.
"""
...
def run_backtest_with_stall_params(stall_enabled, check_bars, min_profit_atr, sl_atr, iteration_name):
    """Run backtest with specific stall parameters using the working replay approach."""
    ...
    # Load config from .env.mtf_v2 (don't modify it)
    load_dotenv(PROJECT_ROOT / ".env.mtf_v2", override=True)
    cfg = load_mtf_v2_config()
    
    # Setup paths - exactly like run_backtest_mtf_v2_replay.py
    ...
    # Strategy config - copy all parameters from working replay script
    strategy_config = ImportableStrategyConfig(
        ...
        config={
            ...
            # Override stall parameters for this test
    ...
    # Venue config - exactly like working replay script
    ...
    # Data configs - dual timeframe like working replay script
...
def main():
    print("\n" + "="*80)
    print("POSITIVE STALL DETECTION OPTIMIZATION - REPLAY BACKTEST V2")
    print("="*80)
    print("\nUsing the working run_backtest_mtf_v2_replay.py implementation.")
    print("No modifications to .env.mtf_v2 file.\n")
    ...
    print(f"\nTotal parameter combinations: {len(combinations) + 1}")
    print(f"Estimated time: {(len(combinations) + 1) * 15} minutes (~15 min per test)\n")
    ...
    baseline_result = run_backtest_with_stall_params(
        stall_enabled=False,
        check_bars=6,
        min_profit_atr=0.2,
        sl_atr=0.2,
    ...
    for idx, (check_bars, min_profit, sl_atr) in enumerate(combinations, 1):
        ...
        result = run_backtest_with_stall_params(
            stall_enabled=True,
