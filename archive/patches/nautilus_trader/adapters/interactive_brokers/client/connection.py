...
class InteractiveBrokersClientConnectionMixin(BaseMixin):
    """Patched connection mixin to prevent premature asyncio cancellation."""

    async def _connect(self) -> None:
        """Establish the socket connection with TWS/Gateway."""
        try:
            # Explicit log to confirm patched method is active
            self._log.info("[PATCH] Starting shielded IB _connect()")
            ...
            async def _handshake() -> None:
                ...
                self._log.info(
                    f"Connected to Interactive Brokers (v{self._eclient.serverVersion_}) at {conn_time_str} "
                    f"from {self._host}:{self._port} with client id: {self._client_id}"
                )

            task = asyncio.create_task(_handshake(), name="ib-handshake")
            try:
                await asyncio.shield(task)
            except asyncio.CancelledError:
                # Outer cancellation should not interrupt handshake; wait for it to finish
                self._log.warning("[PATCH] _connect() was cancelled; waiting for handshake task to finish")
                try:
                    await task
                except Exception:
                    # Propagate errors from handshake to caller so startup can retry
                    raise
                else:
                    # Handshake completed successfully despite cancellation; proceed
                    return
            ...
            self._log.exception(f"Connection failed: {exc}")
            if self._eclient.wrapper:
                self._eclient.wrapper.error(
                    NO_VALID_ID,
                    CONNECT_FAIL.code(),
    ...
    async def _disconnect(self) -> None:
        ...
        try:
            ...
            self._log.exception("Disconnection failed", exc)
    ...
    async def _connect_socket(self) -> None:
        """Connect the socket to TWS / Gateway and change the state to CONNECTING."""
    ...
    def _connect_socket_safe(self) -> None:
        try:
            ...
            raise ConnectionError("Failed to connect to TWS/Gateway.") from exc
