"""
Run MTF V2 backtest with Loosened Entry Confirmation logic.

This is a modified version of run_backtest_mtf_v2_entry_confirmed.py that incorporates
adjustments to the entry confirmation logic to improve performance by loosening criteria.
"""
...
def run_v2_entry_confirmed_loosened_backtest(
    symbol: str = "EUR/USD",
    venue: str = "IDEALPRO",
    ...
    """
    Run V2 backtest with loosened entry confirmation logic.
    ...
    entry_confirmation_bars = _env_int("MTF2_ENTRY_CONFIRM_BARS", 3)  # Adjusted from 2 to 3 for a slightly longer confirmation window
    entry_confirmation_threshold = _env_float("MTF2_ENTRY_CONFIRM_THRESHOLD", 0.15)  # Reduced from 0.2 to 0.15 to loosen criteria
    entry_max_wait_bars = _env_int("MTF2_ENTRY_CONFIRM_MAX_WAIT_BARS", 5)
    high_confidence_bypass_threshold = _env_float("MTF2_HIGH_CONFIDENCE_BYPASS_THRESHOLD", 0.8)  # New parameter for bypassing strict confirmation
    ...
    print("=" * 80)
    print("MTF V2 REPLAY BACKTEST - LOOSENED ENTRY CONFIRMED VERSION")
    print("=" * 80)
    ...
    output_dir = PROJECT_ROOT / "backtest_results" / f"MTF_V2_ENTRY_CONFIRMED_LOOSENED_{timestamp}"
    ...
    strategy_config = ImportableStrategyConfig(
        ...
        config={
            ...
            "high_confidence_bypass_threshold": high_confidence_bypass_threshold,  # New parameter for high confidence bypass
    ...
    try:
        ...
        if not summary_path.exists():
            with open(summary_path, "w", encoding="utf-8") as f:
                f.write("Entry Confirmed V2 Loosened Backtest Results\n")
                f.write("=" * 50 + "\n")
...
def main():
    ...
    # Run backtest
    result, results_dir = run_v2_entry_confirmed_loosened_backtest(
        symbol=config.symbol,
        venue=config.venue,
    ...
    print()
    print("=" * 80)
    print("V2 ENTRY CONFIRMED LOOSENED BACKTEST COMPLETED")
    print("=" * 80)
    ...
    print("3. If improved, consider updating live trading parameters or strategy")
