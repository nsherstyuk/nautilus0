"""
IBKRExecutionEngine - Live execution via Interactive Brokers.

Implements the ExecutionEngine interface for live trading.

Architecture (two-phase brackets, matching V5 production):
    Phase 1: Place ONLY 2 entry stops (OCA pair, no children).
    Phase 2: Place SL/TP AFTER fill confirmed.
    SL/TP are independent OCA orders, not bracket children.
    Cancel by contract conId match (not saved order ID).
    _cancel_all_for_contract() runs before recording every exit.
"""
import logging
import math
...
from ..core.market_event import Fill, RangeInfo
