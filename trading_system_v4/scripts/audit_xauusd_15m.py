"""
audit_xauusd_15m.py — Comprehensive XAUUSD Signal Audit (15m timeframe)

Full audit pipeline to determine if XAUUSD has tradeable edge at 15m:

  A. Resample 1000-tick bars → 15m OHLCV (label='right', closed='right')
  B. Feature engineering: 32 standard + swing_proximity + gold_session
  C. ATR-based TP/SL labeling (TP=1.5×ATR, SL=1.4×ATR, lookahead=100 bars)
...
Comparison baseline: EURUSD 15m AUC = 0.519 (proven zero edge)
Decision threshold : AUC > 0.54 → worth investigating further
...
SPREAD_EST = 0.30      # ~$0.30 gold spread (in USD/oz price terms)
BE_WR      = SL_ATR / (SL_ATR + TP_ATR)   # ~48.3% break-even

# ── CV parameters ─────────────────────────────────────────────────────────────
PURGE_GAP  = 120       # 30 hours gap between train/test
ZSCORE_W   = 96        # 24 hours for z-score normalisation

# ── Helpers ───────────────────────────────────────────────────────────────────

def _atr14(h: np.ndarray, l: np.ndarray, c: np.ndarray) -> np.ndarray:
    """ATR(14) using Wilder's smoothing. Output in USD/oz."""
...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION A: Resample tick bars → 15m
# ══════════════════════════════════════════════════════════════════════════════

def load_and_resample():
    """Load XAUUSD tick bars and resample to 15m OHLCV + microstructure aggregates."""
    print("=" * 70)
    print("  SECTION A: LOAD & RESAMPLE XAUUSD TICK BARS → 15m")
    print("=" * 70)
    ...
    # Resample: label='right', closed='right' — NO lookahead
...
def add_standard_features(df: pd.DataFrame) -> pd.DataFrame:
    """32 standard features + gold-specific features (swing_proximity, gold_session)."""
    print("\n" + "=" * 70)
    ...
    feat['return_5']  = c.pct_change(5, fill_method=None)
    feat['return_12'] = c.pct_change(12, fill_method=None)
    feat['return_24'] = c.pct_change(24, fill_method=None)
    ...
    # Distance from last confirmed local high/low (10-bar lookback, no lookahead)
    # A confirmed swing high at bar i-5: high[i-5] is the max of bars i-10..i
    # (requires 5 bars after to confirm)
    swing_high = h.rolling(10, center=False).max().shift(5)  # confirmed N bars ago
    ...
    # London+NY overlap (13:00-17:00 UTC) — gold's peak liquidity window
    feat['gold_session'] = ((hour >= 13) & (hour < 17)).astype(float)
    # Asian session (dry for gold)
...
def label_tp_sl(df: pd.DataFrame):
    """Simulate TP/SL for every 15m bar. Returns y_long, y_short arrays."""
    print("\n" + "=" * 70)
    print("  SECTION C: ATR-BASED TP/SL LABELING")
...
def purged_cv(X: np.ndarray, y: np.ndarray, n_splits: int = 4,
    ...
    Train on all earlier data (with purge gap), test on next segment.
    """
    n = len(X)
    ...
    for fold in range(n_splits):
        ...
        # Win rate at various probability thresholds
        wr_info = []
    ...
    if results:
        ...
        print(f"\n  → {label} AVERAGE AUC: {avg_auc:.4f}  "
              f"(baseline 0.50, EURUSD was 0.519)")
...
def run_xgb_cv(feat: pd.DataFrame, y_long: np.ndarray, y_short: np.ndarray):
    """Section D: Run purged XGBoost CV for both LONG and SHORT."""
    print("\n" + "=" * 70)
    print("  SECTION D: XGBOOST PURGED WALK-FORWARD CV (STANDARD FEATURES)")
    print("=" * 70)
    ...
    # Skip warmup (200 bars for EMA200)
    warmup = 200
    X = X[warmup:]
    y_l = y_long[warmup:]
    y_s = y_short[warmup:]
...
def session_breakout_test(df: pd.DataFrame, y_long: np.ndarray, y_short: np.ndarray):
    """
    Test if gold session transitions produce edge.
    Gold sessions:
      Asian:  00:00-07:00 UTC
      London: 07:00-13:00 UTC
      NY:     13:00-22:00 UTC
    Key window: London open breakout (07:00-10:00 UTC)
    """
    print("\n" + "=" * 70)
    print("  SECTION E: GOLD SESSION BREAKOUT TEST")
    print("=" * 70)
    ...
    # London open breakout: direction bias test
    # If gold tends to break out of the Asian range during London open,
    # the LONG or SHORT WR should deviate significantly from overall
    ...
    # Yearly consistency check for best session
...
def build_microstructure_features(df: pd.DataFrame) -> pd.DataFrame:
    """Build 62+ microstructure features from tick-bar aggregates."""
    print("\n" + "=" * 70)
    print("  SECTION F: MICROSTRUCTURE ML TEST")
    print("=" * 70)
    ...
    # Z-scores (rolling normalisation)
...
def run_microstructure_cv(micro_feat: pd.DataFrame,
    ...
    """Run purged CV on microstructure features."""
    ...
    if results_long:
        ...
        # Flag if time features dominate (same conclusion as EURUSD)
        time_cols = {"hour", "minute", "dow", "is_london", "is_ny",
        ...
        if time_in_top5 >= 3:
            ...
            print(f"    Same conclusion as EURUSD — model learns session patterns, not microstructure")
...
def print_verdict(std_long, std_short, micro_long, micro_short, session_results):
    print("\n" + "=" * 70)
    print("  SECTION G: FINAL VERDICT — XAUUSD 15m SIGNAL AUDIT")
    print("=" * 70)

    all_aucs = {}

    if std_long:
        auc = np.mean([r["auc"] for r in std_long])
        all_aucs["Standard LONG"] = auc
    if std_short:
        auc = np.mean([r["auc"] for r in std_short])
        all_aucs["Standard SHORT"] = auc
    if micro_long:
        auc = np.mean([r["auc"] for r in micro_long])
        all_aucs["Microstructure LONG"] = auc
    if micro_short:
        auc = np.mean([r["auc"] for r in micro_short])
        all_aucs["Microstructure SHORT"] = auc

    print(f"\n  {'Test':<25s} {'Avg AUC':>8s} {'vs Random':>10s} {'vs EURUSD':>10s}")
    print("  " + "-" * 55)
    for name, auc in all_aucs.items():
        vs_rand = (auc - 0.5) * 100
        vs_eur  = (auc - 0.519) * 100
        print(f"  {name:<25s} {auc:>8.4f} {vs_rand:>+9.1f}pp {vs_eur:>+9.1f}pp")
    ...
    print(f"\n  {'─' * 55}")
    if max_auc < 0.52:
        print("  VERDICT: ❌ ZERO EDGE on XAUUSD 15m")
        print("  Same conclusion as EURUSD — no tradeable signal at this timeframe.")
        print("  RECOMMENDED NEXT STEP: Test XAUUSD at daily timeframe")
        print("  (daily gold trend-following is a documented CTA edge)")
    elif max_auc < 0.54:
        print("  VERDICT: ⚠️  MARGINAL SIGNAL on XAUUSD 15m")
        print(f"  Max AUC: {max_auc:.4f} — probably noise, but worth a second look")
        print("  CONSIDER: Test with larger TP/SL ratios or different timeframes")
    else:
        print("  VERDICT: ✅ POTENTIAL EDGE DETECTED on XAUUSD 15m")
        print(f"  Max AUC: {max_auc:.4f} — exceeds threshold, warrants deeper investigation!")
        print("  NEXT STEP: Walk-forward PnL simulation + spread sensitivity analysis")
