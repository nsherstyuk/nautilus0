...
OUTPUT_FILE = DATA_DIR / "meta_labeled_1000t.parquet"
...
def run():
    ...
    print("Calculating Base Strategy (EMA Crossover) ...")
    df["ema_fast"] = _ema(df["close"], EMA_FAST)
    df["ema_slow"] = _ema(df["close"], EMA_SLOW)
    df["atr"] = _atr(df, 14)

    # 2. Find Crossovers (The Base Signals)
    # 1 = Bullish Cross (Fast > Slow), -1 = Bearish Cross (Fast < Slow), 0 = No Cross
    prev_fast = df["ema_fast"].shift(1)
    prev_slow = df["ema_slow"].shift(1)
    
    bull_cross = (df["ema_fast"] > df["ema_slow"]) & (prev_fast <= prev_slow)
    bear_cross = (df["ema_fast"] < df["ema_slow"]) & (prev_fast >= prev_slow)
    ...
    print(f"Found {len(signal_indices):,} total EMA crossovers.")
    ...
    # If price is going up (bull cross) but volume imbalance is negative (more selling), that's a divergence
    ...
    drop_cols = ["open", "high", "low", "close", "ema_fast", "ema_slow", "atr"]
