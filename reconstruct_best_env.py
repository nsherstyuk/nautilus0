"""
Reconstruct the optimal .env file based on the best backtest result (14k PnL).
Best result: $14,203.91 PnL, 60% win rate, 85 trades
Source: optimization/results/multi_tf_combined_results_summary.json (run_id: 28)
"""
from pathlib import Path

# Best configuration from optimization results
BEST_CONFIG = {
    ...
    "BACKTEST_BAR_SPEC": "1-MINUTE-MID-EXTERNAL",  # Not in JSON, using default from env.example
    ...
    # Risk management (from best result)
    ...
    # Other filters (disabled by default)
    "STRATEGY_RSI_ENABLED": "false",
    "STRATEGY_VOLUME_ENABLED": "false",
    "STRATEGY_ATR_ENABLED": "false",
    
    # Time filter (not in optimization, but recommended from analysis)
    "BACKTEST_TIME_FILTER_ENABLED": "false",  # Set to true if you want to exclude hours
    "BACKTEST_EXCLUDED_HOURS": "1,2,12,18,21,23",  # Recommended hours to exclude
    
    # Market regime detection (disabled - not in best result)
...
def create_env_file(output_path=".env.best"):
    """Create the optimal .env file."""
    env_content = """# NautilusTrader Optimal Configuration
...
"""
    ...
    print(f"[OK] Created optimal .env file: {output_file}")
    print(f"\nBest Configuration Summary:")
    print(f"  - PnL: $14,203.91")
    print(f"  - Win Rate: 60%")
    print(f"  - Trade Count: 85")
    print(f"  - Fast MA Period: 42")
    print(f"  - Slow MA Period: 270")
    print(f"  - Stop Loss: 35 pips")
    print(f"  - Take Profit: 50 pips")
    print(f"  - Trailing Activation: 22 pips")
    print(f"  - Trailing Distance: 12 pips")
    print(f"  - Crossover Threshold: 0.35 pips")
    print(f"  - DMI: Enabled (period 10)")
    print(f"  - Stochastic: Enabled (K=18, D=3, thresholds 30/65)")
    print(f"\nTo use this configuration:")
    print(f"  1. Copy {output_file} to .env")
    print(f"  2. Adjust dates if needed")
