"""
V8 Walk-Forward Validation

Rule-based strategy: walk-forward verifies edge stability across sequential
non-overlapping time windows. Reports per-window stats and detects regime
degradation. Also runs multi-instrument test if EURUSD data exists.

Usage:
    python -m v8_confirmed_rebreak.backtest.walk_forward [--pw 60] [--confirm 3]
    python -m v8_confirmed_rebreak.backtest.walk_forward --multi
"""
import argparse
import io
import contextlib
...
def run_window(symbol: str, start: str, end: str, pw: int, confirm: int,
               ...
               label: str = "", preloaded_df=None) -> Optional[WindowResult]:
...
def generate_windows(first_year: int, last_year: int) -> List[tuple]:
    windows = []
    year = first_year
    half = 1
    while True:
...
def run_multi_instrument(pw: int, confirm: int, hold: int, sl: float,
    ...
    for symbol in symbols:
        ...
        df = BacktestRunner.load_csv(data_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="V8 Walk-Forward Validation & Multi-Instrument Test")
    ...
    full_df = BacktestRunner.load_csv(data_path)
